...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--prefix', default="root://cmseos.fnal.gov:1094//store/group/lpcpfnano/anovak/", help='')
    ...
    for key in sorted(sample_dict.keys()):
        ...
        for i, fname in enumerate(sample_dict[key]):
            ...
            #out = args.prefix + os.path.join(args.dir, fname.split("/")[-1].lstrip("/"))
            out = args.prefix + os.path.join(args.dir, *fname.split("/")[-2:])
            # print(out)
            # quit()
            new_list.append(out)
            if args.run:
